...
* Since this device supports remote control via REST API, MODBUS, etc., utmost attention must be paid to network security management. Users are responsible for maintaining the highest level of network security systems, such as establishing internal factory firewalls, operating a private network, and restricting IP access, to prevent unauthorized access. The manufacturer shall not be held liable for any malfunction or accident of the robot caused by vulnerabilities in the external network environment, negligence in management, hacking, or accidental external command inputs, unless they are due to functional defects inherent to the product itself.
